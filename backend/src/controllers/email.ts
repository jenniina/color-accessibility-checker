...
export const sendEmailForm = async (req: Request, res: Response) => {
  ...
  const sanitizedEncouragement = sanitizeHtml(body.encouragement)
  const sanitizedClarification = sanitizeHtml(body.clarification)
  ...
  let mailOptions = {
    ...
    Color: ${body.color}
    Preference: ${body.dark}${body.light}
    Select Multi: ${body.selectmulti}
